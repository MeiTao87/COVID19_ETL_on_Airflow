...
def covid_etl(country):
    if country is None:
        country = 'India'
    database_location = "sqlite:///covid19.sqlite"
    # download data from 2 days ago
    today = datetime.date.today()
    download_day = (today - datetime.timedelta(days=2)).strftime('%m-%d-%Y')
    download_day = datetime.datetime.strptime(download_day, "%m-%d-%Y")
    day_str = "%02d-%02d-%d" % (download_day.month, download_day.day, download_day.year)
